package org.exoplatform.setting.server.util;
    ...
    try {
      ...
      if(in != null) {
        try {
          ...
          // Skip beginning of the file, only streaming is interesting 
          in.skip(file.length());
          ...
          while (!Thread.interrupted()) {
              ...
              if (len > 0) {
                ...
                for(String line : lines) {
                  ...
                  if(line.matches(".*(" + WizardUtility.getTailEndWordsRegexp(WizardUtility.getCurrentServerName()) + ").*")) {
                    logger.debug("Thread interruption because end of logs (" + pathToFile +  ")");
            ...
            try {
              ...
              // Tail is automatically interrupted if sleep again after max tail sleep property
              totalTailSleep += MS_TAIL_SLEEP;
              if(totalTailSleep > WizardProperties.getMaxTailSleep()) {
                logger.debug("Thread interruption because time execution was exceeded (" + pathToFile + ")");
                Thread.currentThread().interrupt();
              }
