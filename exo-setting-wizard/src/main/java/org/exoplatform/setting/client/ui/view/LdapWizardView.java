...
public class LdapWizardView extends WizardView {
  ...
  public LdapWizardView(SetupWizardController controller, SetupWizardMode mode) {
    super(controller, SetupScreen.IDM_LDAP);
  ...
  public Map<SetupWizardData, String> verifyDatas() throws WizardInvalidFieldException {
    ...
    if(! WizardFieldVerifier.isValidTextField(providerUrl.getText())) {
      throw new WizardInvalidFieldException(constants.invalidProviderUrl());
    }

    if(! WizardFieldVerifier.isValidTextField(baseDn.getText())) {
      throw new WizardInvalidFieldException(constants.invalidBaseDn());
    }

    if(! WizardFieldVerifier.isValidTextField(rootDn.getText())) {
      throw new WizardInvalidFieldException(constants.invalidRootDn());
    }
  ...
  public void preFillFields() {
    providerUrl.setText("ldap://hostname:10389");
    baseDn.setText("dc=example,dc=com");
    rootDn.setText("ui=admin,ou=system");
